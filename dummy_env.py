...
class VecEnv(ABC):
    ...
    - "time_outs" (torch.Tensor): Timeouts for the environments. These correspond to terminations that happen due to
      time limits and not due to the environment reaching a terminal state. This is useful for environments that have a
    ...
    @abstractmethod
    def get_init_observations(self) -> tuple[torch.Tensor, dict]:
